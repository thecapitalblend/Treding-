...
class MasterAggregator:
    ...
    def analyze(self, df, symbol, news_query, enable_finbert=False):
        ...
        sentiment = self.sent_engine.analyze(
            news_query, enable_finbert=enable_finbert
        )

        # Use the actual current instant. The astro engine converts IST -> UTC
        # internally and calculates sidereal Lahiri positions.
        celestial = self.astro_engine.calculate()

        technical_score = float(tech["score"])
        sentiment_score = float(sentiment["score"])
        celestial_score = float(celestial["score"]) if celestial["available"] else 0.0

        composite = (
            0.65 * (technical_score / 100.0)
            + 0.25 * (sentiment_score / 100.0)
            + 0.10 * (celestial_score / 5.0)
        )
        confidence = float(np.clip(composite * 100.0, -100, 100))

        # Celestial layer is confirmatory only; it cannot create a trade
        # without sufficient technical confirmation.
        ...
        atr = (
            float(tech["atr"])
            if np.isfinite(tech["atr"]) and tech["atr"] > 0
            else max(price * 0.003, 1.0)
        )

        direction = (
            "CALL / LONG" if signal == "BUY CALL"
            else "PUT / SHORT" if signal == "BUY PUT"
            else "WAIT"
        )

        if signal == "BUY CALL":
            entry, sl = price, price - 1.2 * atr
            t1, t2 = price + 1.5 * atr, price + 2.5 * atr
        elif signal == "BUY PUT":
            entry, sl = price, price + 1.2 * atr
            t1, t2 = price - 1.5 * atr, price - 2.5 * atr
        else:
